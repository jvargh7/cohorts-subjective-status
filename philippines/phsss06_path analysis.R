...
source(paste0(path_sss_repo,"/philippines/phaux01_renaming variables.R"))
source(paste0(path_sss_repo,"/philippines/phaux02_path models.R"))
path_df <- readRDS(paste0(path_dissertation,"/aim 3/working/cohorts/philippines/path_df.RDS"))
...
all_models %>% 
  ph_rename_sss(.) %>% 
  write.csv(.,paste0(path_dissertation,"/aim 3/working/cohorts/philippines/path analysis summary.csv"),row.names = FALSE)
...
all_models %>% 
  dplyr::filter(dv %in% c(ses_vars,ladder_vars),mediator == "Community Respect",outcome=="BMI in 2018") %>% 
  mutate(iv = case_when(iv %in% ladder_vars ~ "adladder",
                        TRUE ~ iv)) %>% 
  ph_rename_sss(.) %>%
  ...
  ylab("") + xlab("Estimate (95% CI)") +
  # theme(legend.position = "bottom") +
  facet_wrap(~dv,scales = "free_x",nrow = 3,ncol=4)

all_models %>% 
  dplyr::filter(dv %in% c(ses_vars,ladder_vars),mediator == "Economic Status",outcome=="BMI in 2018") %>% 
  mutate(iv = case_when(iv %in% ladder_vars ~ "adladder",
                        TRUE ~ iv)) %>% 
  ph_rename_sss(.) %>%
  mutate(dv = dv[order(match(dv,ses_vars))]) %>% 
