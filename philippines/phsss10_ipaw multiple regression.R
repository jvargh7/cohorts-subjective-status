...
source(paste0(path_sss_repo,"/package/retrieve_alive_df.R"))
...
for (y in c("bmi","srq","happiness")) {
  ...
  temp_path_mr <- temp_mr(y,dfs = sss_mi_dfs,site = "philippines",
                          ipaw = TRUE)
...
result_path_mr %>% 
  write.csv(.,paste0(path_dissertation,"/aim 3/working/cohorts/philippines/multiple regression ipaw coefficients.csv"),row.names = FALSE)
