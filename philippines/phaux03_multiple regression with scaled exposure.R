...
for (y in c("bmi","srq","happiness")) {
  ...
  temp_path_mr_mice <- mice_pool_reg(y,df = sss_mi_dfs,site = "philippines",scaled=TRUE)
...
result_path_mr %>% 
  write.csv(.,paste0(path_dissertation,"/aim 3/working/cohorts/philippines/multiple regression scaled exposure coefficients.csv"),row.names = FALSE)
