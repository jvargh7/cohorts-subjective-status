...
gt_df <- read_csv(paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/multiple regression all wealth coefficients.csv")) %>% 
...
ph_df <- read_csv(paste0(path_dissertation,"/aim 3/working/cohorts/philippines/multiple regression all wealth coefficients.csv")) %>% 
  pivot_longer(cols=contains("model"),names_to="model",values_to="coef_ci") %>% 
  separate(coef_ci,into=c("coef","lci","uci"),sep="(\\(|,)") %>% 
  dplyr::mutate(uci = str_replace(uci,"\\)","")) %>% 
  mutate_at(vars(coef,lci,uci),~as.numeric(.)) %>% 
...
sa_df <- read_csv(paste0(path_dissertation,"/aim 3/working/cohorts/south africa/multiple regression all wealth coefficients.csv")) %>% 
  pivot_longer(cols=contains("model"),names_to="model",values_to="coef_ci") %>% 
  separate(coef_ci,into=c("coef","lci","uci"),sep="(\\(|,)") %>% 
  dplyr::mutate(uci = str_replace(uci,"\\)","")) %>% 
  mutate_at(vars(coef,lci,uci),~as.numeric(.)) %>% 
