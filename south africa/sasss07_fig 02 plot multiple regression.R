...
result_path_mr <- read_csv(paste0(path_dissertation,"/aim 3/working/cohorts/south africa/multiple regression coefficients.csv")) %>% 
...
result_mr_contrasts <- read_csv(paste0(path_dissertation,"/aim 3/working/cohorts/south africa/multiple regression heterogeneity contrasts table.csv")) %>% 
  ...
  dplyr::mutate(uci = str_replace(uci,"\\)","")) %>% 
  mutate_at(vars(coef,lci,uci),~as.numeric(.)) %>% 
  mutate(variable = case_when(y == "bmi" ~ "BMI in 2012",
