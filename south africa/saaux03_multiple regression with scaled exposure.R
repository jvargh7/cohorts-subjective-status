...
for (y in c("srq","happiness")) {
  ...
  temp_path_mr_mice <- mice_pool_reg(y,df = sss_mi_dfs,site = "south africa",scaled =TRUE)
...
result_path_mr %>% 
  write.csv(.,paste0(path_dissertation,"/aim 3/working/cohorts/south africa/multiple regression scaled exposure coefficients.csv"),row.names = FALSE)
