...
source(paste0(path_sss_repo,"/south africa/saaux01_renaming variables.R"))
source(paste0(path_sss_repo,"/south africa/saaux02_path models.R"))
path_df <- readRDS(paste0(path_dissertation,"/aim 3/working/cohorts/south africa/path_df.RDS"))
...
all_models %>% 
  sa_rename_sss(.) %>% 
  write.csv(.,paste0(path_dissertation,"/aim 3/working/cohorts/south africa/path analysis summary.csv"),row.names = FALSE)

# Plot ----------

all_models %>% 
  dplyr::filter(iv %in% c(ses_vars,ladder_vars),dv %in% c("srq","happiness")) %>% 
...
all_models %>% 
  dplyr::filter(dv %in% c(ses_vars,ladder_vars),mediator == "Community Respect",outcome=="SRQ-20 in 2018") %>% 
  mutate(iv = case_when(iv %in% ladder_vars ~ "adladder",
                        TRUE ~ iv)) %>% 
  sa_rename_sss(.) %>%
  ...
  ylab("") + xlab("Estimate (95% CI)") +
  # theme(legend.position = "bottom") +
  facet_wrap(~dv,scales = "free_x",nrow = 3,ncol=3)

all_models %>% 
  dplyr::filter(dv %in% c(ses_vars,ladder_vars),mediator == "Economic Status",outcome=="SRQ-20 in 2018") %>% 
  mutate(iv = case_when(iv %in% ladder_vars ~ "adladder",
                        TRUE ~ iv)) %>% 
  sa_rename_sss(.) %>%
  mutate(dv = dv[order(match(dv,ses_vars))]) %>% 
