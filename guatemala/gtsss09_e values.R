...
sd_y <- readRDS(paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/sss_df.RDS")) %>% 
  summarize_at(vars(bmi,srq,happiness),~sd(.,na.rm=TRUE)) %>% 
  pivot_longer(cols=everything(),names_to = "variable",values_to="sd") %>% 
  mutate(variable = case_when(variable == "bmi" ~ "BMI in 2016",
...
evalue_df <- read_csv(paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/multiple regression coefficients.csv")) %>% 
...
write.csv(evalue_df,paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/evalues.csv"),row.names = FALSE)
