...
for (y in c("bmi","srq","happiness")) {
  ...
  temp_path_mr_mice <- mice_pool_reg(y,df = sss_mi_dfs,site = "guatemala",all_wealth = TRUE)
...
result_path_mr %>% 
  write.csv(.,paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/multiple regression all wealth coefficients.csv"),row.names = FALSE)
