...
result_path_mr <- read_csv(paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/multiple regression coefficients.csv")) %>% 
...
result_mr_contrasts <- read_csv(paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/multiple regression heterogeneity contrasts table.csv")) %>% 
  ...
  dplyr::mutate(uci = str_replace(uci,"\\)","")) %>% 
  mutate_at(vars(coef,lci,uci),~as.numeric(.)) %>% 
  mutate(variable = case_when(y == "bmi" ~ "BMI in 2016",
...
bind_rows(result_path_mr,
  ...
  # dplyr::filter(!(sss=="Economic Status"&variable=="Happiness in 2018")) %>% 
