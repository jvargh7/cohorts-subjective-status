...
for (y in c("bmi","srq","happiness")) {
  print(y)
  temp_path_mr <- temp_mr(o = y,dfs = sss_mi_dfs,
                          site = "guatemala",
                          ipaw = TRUE)
  # y_models[y] <- temp_path_mr[[2]]
  
  # temp_path_mr_mice <- mice_pool_reg(y,df = sss_mi_dfs,site = "guatemala",
  #                                    scaled = FALSE,ipaw = TRUE)
...
result_path_mr %>% 
  write.csv(.,paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/multiple regression ipaw coefficients.csv"),row.names = FALSE)
