...
for (y in vars_y) {
  for (h in vars_het){
    ...
    heterogeneity_out <- consolidated_heterogeneity(y,miaux_dfs = sss_mi_dfs,het_var = h,site="guatemala")
    
    # Check using mice
    heterogeneity_out_mice <- mice_heterogeneity(y,miaux_dfs = sss_mi_dfs,het_var = h,site="guatemala")
...
consolidated_summary_table %>% 
  write.csv(.,paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/multiple regression heterogeneity summary table.csv"),row.names = FALSE)
consolidated_contrasts_table %>% 
  write.csv(.,paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/multiple regression heterogeneity contrasts table.csv"),row.names = FALSE)
consolidated_nested_table %>% 
  write.csv(.,paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/multiple regression heterogeneity nested table.csv"),row.names = FALSE)
