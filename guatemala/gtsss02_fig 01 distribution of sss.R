...
gt_fig01_df <- readRDS(paste0(path_dissertation,"/aim 3/working/cohorts/guatemala/sss_df.RDS"))
...
ur_c <- gt_fig01_df %>% 
  ggplot(data=.,aes(x=factor(male,labels=c("Female","Male")),
                    y=adladdercommunity,fill=factor(rural2018,labels=c("Urban","Rural")))) +
  geom_boxplot() + 
  theme_bw() +
  xlab("") +
...
ur_e <- gt_fig01_df %>% 
  ggplot(data=.,aes(x=factor(male,labels=c("Female","Male")),
                    y=adladdereconomic,fill=factor(rural2018,labels=c("Urban","Rural")))) +
  geom_boxplot() + 
  theme_bw() +
  xlab("") +
